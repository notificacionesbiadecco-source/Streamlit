...
def cargar_preguntas():
    try:
        ...
        while True:
            response = supabase.table('encuestas').select('*').eq('formulario', 'Tercera').range(start, start + page_size - 1).execute()
...
st.title("📝 Encuesta - Tercera Evaluación")
...
if not preguntas_data:
    st.warning("⚠️ No se encontraron preguntas para la encuesta 'Tercera Evaluación'")
    st.stop()
...
with col2:
    ...
    if cedula_input_evaluado.strip():
        ...
        if usuario_evaluado:
            ...
            # ✅ VALIDAR ENCUESTA "PRIMERA" (8 DÍAS DE ESPERA)
            try:
                # response = supabase.table('resultados_encuestas')\
                #     .select('created_at')\
                #     .eq('cedula_evaluado', int(usuario_evaluado['cedula']))\
                #     .eq('tipo_encuesta', 'Primera')\
                #     .order('created_at', desc=True)\
                #     .limit(1)\
                #     .execute()

                # if not response.data:
                #     st.error("❌ Este usuario NO ha completado la encuesta **'Primera'**")
                #     st.warning("⚠️ Debe completar primero la encuesta 'Primera' antes de acceder a la 'Tercera'")
                #     st.stop()
                # else:
                #     fecha_primera_encuesta_str = response.data[0]['created_at']
                #     fecha_primera_encuesta = datetime.fromisoformat(fecha_primera_encuesta_str.replace('Z', '+00:00'))
                #     fecha_primera_encuesta_local = fecha_primera_encuesta.astimezone().date()
                #     fecha_actual = datetime.now().date()
                #     dias_desde_primera = (fecha_actual - fecha_primera_encuesta_local).days

                #     # if dias_desde_primera < 8:
                #     #     dias_faltantes = 8 - dias_desde_primera
                #     if dias_desde_primera <= 0:
                #         dias_faltantes = 0 - dias_desde_primera
                #         st.warning(f"⏳ **Este usuario completó la 'Primera' encuesta hace {dias_desde_primera} día(s)**")
                #         st.error(f"❌ Debe esperar **{dias_faltantes} día(s) más** para poder realizar la 'Tercera' encuesta")
                #         st.info(f"📅 Fecha de la Primera encuesta: **{fecha_primera_encuesta_local.strftime('%d/%m/%Y')}**")
                #         st.info(f"📅 Podrá realizar la Tercera encuesta a partir del: **{(fecha_primera_encuesta_local + timedelta(days=8)).strftime('%d/%m/%Y')}**")
                #         st.stop()
                #     else:
                #         st.success(f"✅ Han pasado **{dias_desde_primera} días** desde la Primera encuesta")
                #         st.info(f"📅 Fecha de la Primera encuesta: **{fecha_primera_encuesta_local.strftime('%d/%m/%Y')}**")

                #         # # ✅ VERIFICAR SI YA RESPONDIÓ LA "TERCERA" ENCUESTA
                #         # response_tercera = supabase.table('resultados_encuestas')\
                #         #     .select('created_at')\
                #         #     .eq('cedula_evaluado', int(usuario_evaluado['cedula']))\
                #         #     .eq('tipo_encuesta', 'Tercera')\
                #         #     .execute()

                #         # if response_tercera.data:
                #         #     st.warning("⚠️ Este usuario YA completó la encuesta 'Tercera'")
                #         #     st.info(f"📅 Fecha de completación: **{datetime.fromisoformat(response_tercera.data[0]['created_at'].replace('Z', '+00:00')).astimezone().strftime('%d/%m/%Y %H:%M')}**")
