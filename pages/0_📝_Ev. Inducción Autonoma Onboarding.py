...
st.title("📝 Encuesta - Evaluación Inducción autónoma")
...
def cargar_preguntas():
    try:
        ...
        while True:
            response = supabase.table('encuestas').select('*').eq('formulario', 'Induccion').range(start, start + page_size - 1).execute()
            ...
            # Si recibimos menos registros que el page_size, ya no hay más datos
...
if not preguntas_data:
    st.warning("⚠️ No se encontraron preguntas para la encuesta 'Inducción autónoma'")
    st.stop()
...
with st.form("formulario_encuesta"):
    ...
    if submitted:
        # Validar que se hayan seleccionado ambos usuarios
        if not usuario_evaluado or not usuario_evaluador:
            st.error("⚠️ Debes seleccionar tanto el usuario evaluado como el evaluador")
            st.stop()
