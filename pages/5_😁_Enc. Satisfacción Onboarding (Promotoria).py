...
st.title("📝 Encuesta - Evaluación Satisfacción")
...
def cargar_preguntas():
    try:
        ...
        while True:
            response = supabase.table('encuestas').select('*').eq('formulario', 'Satisfaccion').range(start, start + page_size - 1).execute()
...
if not preguntas_data:
    st.warning("⚠️ No se encontraron preguntas para la encuesta 'Satisfacción'")
    st.stop()
...
with col2:
    ...
    if cedula_input_evaluado.strip():
        ...
        if usuario_evaluado:
            ...
            # ✅ VALIDAR FECHA DE INGRESO (35 DÍAS)
            ...
            if fecha_ingreso:
                ...
                #     dias_faltantes = 35 - dias_transcurridos
                if dias_transcurridos <= 0:
                    ...
                    st.info(f"📅 Podrá ser evaluado a partir del: **{(fecha_ingreso + timedelta(days=35)).strftime('%d/%m/%Y')}**")
                    st.stop()
                else:
...
with st.form("formulario_encuesta"):
    ...
    if submitted:
        ...
        # Validar que el usuario evaluado esté seleccionado
        ...
        #         st.markdown(f"**{icono} Pregunta {idx + 1}:** {datos['pegunta']}")
