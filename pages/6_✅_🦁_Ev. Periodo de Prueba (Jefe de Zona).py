...
def cargar_preguntas():
    try:
        ...
        while True:
            response = supabase.table('encuestas').select('*').eq('formulario', 'Periodo de Prueba Jefe Zona').range(start, start + page_size - 1).execute()
...
col1, col2 = st.columns([1, 2])
with col1:
    cargos_filtrados_evaluador = ['JEFE DE ZONA']
        ...
        try:
            ...
            for idx, datos in respuestas_usuario.items():
                ...
                registro = {
                    ...
                    'puntaje': int(puntaje),
                    'tipo_encuesta': 'Periodo de Prueba Jefe Zona',
