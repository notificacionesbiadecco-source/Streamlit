...
ROLES_PERMITIDOS = ['ENTRENADOR', 'ENTRENADOR MASTER', 'ENTRENADOR JUNIOR', 'ENTRENADOR INTEGRAL']
...
# Permitir acceso si es admin O tiene uno de los roles permitidos
if not st.session_state.is_admin and st.session_state.user_cargo not in ROLES_PERMITIDOS:
    st.error("⛔ No tienes permisos para acceder a esta página")
    st.info(f"Tu rol actual: **{st.session_state.user_cargo}**")
    st.stop()
...
def cargar_preguntas():
    try:
        ...
        while True:
            response = supabase.table('encuestas').select('*').eq('formulario', 'Periodo de Prueba Entrenador').range(start, start + page_size - 1).execute()
...
def cargar_usuarios():
    try:
        cargos = ['ESPECIALISTA DE EXPERIENCIA', 'MERCHANDISER (EXPERT)', 'ENTRENADOR', 'ENTRENADOR MASTER', 'ENTRENADOR JUNIOR', 'ENTRENADOR INTEGRAL', 'JEFE DE ZONA']
        all_data = []
        page_size = 1000
        start = 0
...
st.title("📝 Encuesta - Periodo de Prueba")
...
if not preguntas_data:
    st.warning("⚠️ No se encontraron preguntas para la encuesta 'Periodo de Prueba'")
    st.stop()
...
with col2:
    ...
    if cedula_input_evaluado.strip():
        ...
        if usuario_evaluado:
            ...
            # ✅ VALIDAR FECHA DE INGRESO (45 DÍAS)
            ...
            if fecha_ingreso:
                ...
                #     dias_faltantes = 45 - dias_transcurridos
                if dias_transcurridos <= 0:
                    ...
                    st.info(f"📅 Podrá ser evaluado a partir del: **{(fecha_ingreso + timedelta(days=45)).strftime('%d/%m/%Y')}**")
                    st.stop()
                else:
        ...
        try:
            ...
            for idx, datos in respuestas_usuario.items():
                ...
                registro = {
                    ...
                    'puntaje': int(puntaje),
                    'tipo_encuesta': 'Periodo de Prueba Entrenador',
