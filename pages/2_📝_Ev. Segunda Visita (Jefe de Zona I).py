...
def cargar_preguntas():
    try:
        ...
        while True:
            response = supabase.table('encuestas').select('*').eq('formulario', 'Segunda').range(start, start + page_size - 1).execute()
...
st.title("📝 Encuesta - Segunda Evaluación")
...
if not preguntas_data:
    st.warning("⚠️ No se encontraron preguntas para la encuesta 'Segunda Evaluación'")
    st.stop()
...
with col2:
    ...
    if cedula_input_evaluado.strip():
        ...
        if usuario_evaluado:
            ...
            # Validar fecha de ingreso (mínimo 9 días)
            ...
            if fecha_ingreso:
                ...
                # if dias_transcurridos < 9:
                #     dias_faltantes = 9 - dias_transcurridos
                if dias_transcurridos <= 0:
                    ...
                    st.info(f"📅 Podrá ser evaluado a partir del: **{(fecha_ingreso + timedelta(days=9)).strftime('%d/%m/%Y')}**")
                    st.stop()
                else:
