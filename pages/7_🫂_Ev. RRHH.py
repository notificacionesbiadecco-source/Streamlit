...
def cargar_preguntas():
    try:
        ...
        while True:
            response = supabase.table('encuestas').select('*').eq('formulario', 'RRHH').range(start, start + page_size - 1).execute()
...
if not preguntas_data:
    st.warning("⚠️ No se encontraron preguntas para la encuesta 'RRHH'")
    st.stop()
...
col1, col2 = st.columns([1, 2])
with col1:
    cargos_filtrados_evaluador = ['ENTRENADOR', 'ENTRENADOR MASTER','ENTRENADOR JUNIOR','JEFE DE ZONA']
